...
def extract_chapters(video_url):
    try:
        ...
        print(
            f"Error: Unable to evaluate the chapter data. The output may not be in the correct format. Details: {e}"
        )
        return []
...
def save_song_artist_list(chapters, file_name):
    directory = os.path.expanduser(
        "~/Odysseus (Syncthing)/07 - Archives/02 - Extrapolated Playlists/"
    )
    ...
    with open(full_path, "w") as f:
        ...
        f.write("\nGenerated by PlaylistGen")  # Add a footer
    return full_path
...
def main():
    ...
    if title and uploader:
        # Generate the file name in "Uploader - Title.md" format
        file_name = f"{uploader} - {title}.md"
        ...
        if chapters:
            ...
            print(f"Song-Artist list saved as a markdown file to {full_path}")
            # print("Please check the ~/playlistGen/playlists/ directory for your markdown playlist file.")
        else:
